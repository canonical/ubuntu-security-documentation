...
includes the server daemon's version is often called a "version banner".
...
Threat actors often perform reconnaissance scans as a preliminary step to their
attacks in order to obtain as much data as possible about the software stack of
their target. Version banners could leak information useful to a threat actor to
mount an attack.
...
determine that they should attempt exploits, leading to unnecessary alerts.
These automatic scanners might skip servers that do not expose their versions.
...
advertised publicly or not. Additionally, it is often possible to, with a
certain degree of confidence, derive the version of a network daemon based on
probing and observing its behavior.

A network vulnerability management scanner often relies on the same version
banner that threat actors' scanners use. If these are disabled, the
vulnerability scanner may no longer determine the correct version, which could
lead to a large number of false positives (the scanner reporting vulnerabilities
which the software is not affected by). This could be especially problematic if
a partial version can be determined, but without the Ubuntu patch level, as
explained below.
...
in subsequent upstream versions: 9.8, 9.9p2, etc. In Ubuntu, these fixes were
backported to the 9.6p1 version. At the time of this writing, the latest version
of OpenSSH available in Noble is 9.6p1-3ubuntu13.16.
...
for negotiation. This would exacerbate the problem, as the Ubuntu patch level
would be unlikely to be included in such an exchange (e.g. retaining only
``9.6`` in the OpenSSH example above).
